...
const DetailArtikelPage = async ({ params }: { params: { id: string } }) => {
  ...
  if (!data) {
    console.error("Article detail not found in response");
    return notFound();
  }

  let otherArticles: ArticleDetail[] = [];
  try {
    const otherRes = await fetch(`${process.env.NEXT_PUBLIC_SELLER}/articles`);
    if (otherRes.ok) {
      const otherJson = await otherRes.json();
      const allArticles: ArticleDetail[] = Array.isArray(otherJson.data)
        ? (otherJson.data as ArticleDetail[])
        : [];
      ...
      otherArticles =
        sameCategory.length > 0
          ? sameCategory
          : shuffleArray(allOtherArticles).slice(0, 3);
    }
  } catch (e) {
    ...
    <div>
      ...
      <div className="md:px-[160px]">
        <div className="grid justify-center">
          <div className="p-4">
            <div className="text-gray-500 mb-2 text-[12px] text-center">
            ...
            <div className="flex justify-center">
              {data.imageUrl ? (
                <Image
                  ...
                  width={600}
                  height={400}
                  className="rounded-lg mb-4 md:h-[500px] md:w-[500px]"
                />
              ) : (
                <div className="w-[600px] h-[400px] bg-gray-300 flex items-center justify-center rounded-lg mb-4">
        ...
        <div className="px-[10px]">
          <div className="text-[14px] font-semibold mb-2 text-center md:text-left pt-10 ">
            {otherArticles.some((a) => a.category.name === data.category.name)
              ? `Other Articles in ${data.category.name}`
              : "Other Articles"}
          </div>
          {otherArticles.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:pt-10">
              {otherArticles.map((article) => (
                <div
                  key={article.id}
                  className="bg-white rounded-lg shadow flex flex-col justify-between p-4 space-y-2 h-full"
                >
